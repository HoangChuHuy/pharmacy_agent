...
class QdrantVectorStore:
    def __init__(
        ...
        embedding_api:str = EMBEDDING_API
    ):
        """
        ...
        if self.embedding_model:
            path = os.path.join("models", self.embedding_model)
    ...
    def get_embedding(self, sentences:Union[List[str], str]="", emb_type: Literal['api', 'local']= 'api'):
        if emb_type == "local":
            ...
            data = [sentences] if isinstance(sentences, str) else sentences
            headers = {'accept': 'application/json'}
            response = requests.post(self.embedding_api,json=data, headers=headers)
            if response.status_code == 200:
                return response.json()['data']
            else:
                logger.warning('Error when calling api: {}', response)
                return None
    ...
    def add_documents(self, documents: List[Document]) -> bool:
        ...
        try:
            ...
            points = []
            embeddings = self.get_embedding([doc.page_content for doc in documents])
    ...
    def search(self, query: str, k: int = 5):
        query_embedding = self.get_embedding([query], 'local')[0]
